...
static gchar *
write_file_from_stream(const char *cert_file_name, int contentLength, FCGX_Request request)
{
  bool success = false;
  ...
  if (!strncmp(contentType, MULTIPART_FORM_DATA, sizeof(MULTIPART_FORM_DATA)-1)) {
    char *boundarytext =
    ...
    if (file_des == -1) {
      ...
      while (done < contentLength) {
        ...
        syslog_v(LOG_INFO, "FCGX_GetStr: bytesRead %d, p_payload %p(%d), availableLen %d(%d)", bytesRead, p_payload, (int)(p_payload - buffer), availableLen, availableLen - bufferLen);
        if (bytesRead < 0) {
        ...
        if (post_boundary_found == 0) {
          for (pchar = (counter == 1) ? p_payload: buffer;
              pchar < buffer + bytesRead - ((counter == 1) ? boundarytextLen: 0);
              pchar++) {
            if (memcmp(pchar, boundarytext, boundarytextLen) == 0) {
              syslog_v(LOG_INFO, "Post boundary found for %.*s", boundarytextLen, pchar);
              pchar -= dataendLen;
              if (memcmp(pchar, dataend, dataendLen) != 0) {
                syslog(LOG_ERR, "Post boundary data end not found");
                syslog_v(LOG_INFO,"Found %02x%02x%02x%02x at post boundary", pchar[0], pchar[1], pchar[2], pchar[3]);
                goto end;
              }
        ...
        while (towrite > 0) {
          if ((written = write(file_des, p_payload + written, towrite)) < 0) {
            syslog(LOG_ERR, "Failed to write %d to %s, err %s", towrite, file_path, strerror(errno));
            goto end;
          }
          done += written;
          towrite -= written;
        }
        syslog_v(LOG_INFO, "write: p_payload %p, %d bytes", p_payload, (int)(p_payload_end - p_payload));
        syslog_v(LOG_INFO, "counter %d, bytesRead %d, done %d", counter, bytesRead, done);
    ...
    syslog(LOG_INFO, "Content-Type not currently supported. Try \"%s\"..", MULTIPART_FORM_DATA);
  }
